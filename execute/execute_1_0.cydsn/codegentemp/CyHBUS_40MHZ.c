...
/*******************************************************************************
* Function Name: CyHBUS_40MHZ_Start
********************************************************************************
*
* Summary:
...
void CyHBUS_40MHZ_Start(void) 
{
    ...
    CyHBUS_40MHZ_CLKEN |= CyHBUS_40MHZ_CLKEN_MASK;
	CyHBUS_40MHZ_CLKSTBY |= CyHBUS_40MHZ_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CyHBUS_40MHZ_Stop
********************************************************************************
*
* Summary:
...
void CyHBUS_40MHZ_Stop(void) 
{
    ...
    CyHBUS_40MHZ_CLKEN &= (uint8)(~CyHBUS_40MHZ_CLKEN_MASK);
	CyHBUS_40MHZ_CLKSTBY &= (uint8)(~CyHBUS_40MHZ_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CyHBUS_40MHZ_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CyHBUS_40MHZ_CLKEN & CyHBUS_40MHZ_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CyHBUS_40MHZ_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CyHBUS_40MHZ_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CyHBUS_40MHZ__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CyHBUS_40MHZ_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CyHBUS_40MHZ_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CyHBUS_40MHZ_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CyHBUS_40MHZ_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CyHBUS_40MHZ_GetSourceRegister();
    uint16 oldDivider = CyHBUS_40MHZ_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CyHBUS_40MHZ_CLKEN & CyHBUS_40MHZ_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CyHBUS_40MHZ_DIV_PTR, clkDivider);
                CyHBUS_40MHZ_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CyHBUS_40MHZ_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CyHBUS_40MHZ_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CyHBUS_40MHZ_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: CyHBUS_40MHZ_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CyHBUS_40MHZ_DIV_PTR);
}


/*******************************************************************************
* Function Name: CyHBUS_40MHZ_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CyHBUS_40MHZ_MOD_SRC |= modeBitMask & (uint8)CyHBUS_40MHZ_MODE_MASK;
}


/*******************************************************************************
* Function Name: CyHBUS_40MHZ_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CyHBUS_40MHZ_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CyHBUS_40MHZ_MODE_MASK));
}


/*******************************************************************************
* Function Name: CyHBUS_40MHZ_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CyHBUS_40MHZ_MOD_SRC & (uint8)(CyHBUS_40MHZ_MODE_MASK);
}


/*******************************************************************************
* Function Name: CyHBUS_40MHZ_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CyHBUS_40MHZ_GetDividerRegister();
    uint8 oldSrc = CyHBUS_40MHZ_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CyHBUS_40MHZ_MOD_SRC |= CYCLK_SSS;
        CyHBUS_40MHZ_MOD_SRC =
            (CyHBUS_40MHZ_MOD_SRC & (uint8)(~CyHBUS_40MHZ_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CyHBUS_40MHZ_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CyHBUS_40MHZ_MOD_SRC & CyHBUS_40MHZ_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CyHBUS_40MHZ_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CyHBUS_40MHZ_PHASE = clkPhase & CyHBUS_40MHZ_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CyHBUS_40MHZ_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CyHBUS_40MHZ_PHASE & CyHBUS_40MHZ_PHASE_MASK;
}
